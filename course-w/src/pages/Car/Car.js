...
const CarView = ({car,mode}) =>{
    ...
    return(
        <main className={classes.wrapper}>
            <p className={classes.title}><span className={classes.name}>{car.brand} {car.model}</span> <span className={classes.price}>{car.price} &#8381;</span></p>

            {car.galery ? <Slider arrowClass={classes.arrow} arrowLeftClass={classes.arrowLeft} arrowRightClass={classes.arrowRight} slides_count={mode === "mobile" ? 1:2} wrapClass={classes.sliderWrap} >
                {car.galery.map((img,index)=><img src={img} type={'slide'} key={`car${index}`} alt="car"/>)}
            </Slider>:<img className={classes.mainImg} src={car.img}/> }
...
const Hider = ({children}) =>{
    ...
    return(
        <>
            ...
            <p style={{color:"red",fontWeight:500}} onClick={()=>{setOpen(!isOpen)}}>{isOpen ? "Свернуть":"Развернуть"}</p>
        </>
    )
}